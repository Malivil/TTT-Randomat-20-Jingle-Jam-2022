...
net.Receive("RdmtSecretSantaParanoiaBegin", function()
   ...
   if client:SteamID64() == target then
      -- Generate a unique ID for this pairing and save it to be cleaned up later
      local timerId = "RdmtSecretSantaParanoia_" .. owner .. "_" .. target
      table.insert(timerIds, timerId)

      startTimer(timerId, player.GetBySteamID64(target), delay_min, delay_max)
   end
end)
